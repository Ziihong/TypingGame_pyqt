...
class CMap:
    ...
    def createWord(self):        #hangman 게임에서 secretWord를 지정한 것과 마찬가지로, randint메소드를 사용하여 임의의 수를 뽑고 이 숫자를 문자열의 인덱스를 가리키는 데에 사용하여 self.word에 있는 단어를 무작위로 뽑아낸다
    ...
    def downWord(self, speed):      #단어가 일정 속도를 유지하며 아래로 떨어질 수 있도록 한다
        ...
        for w in self.word[:]:                  #단어가 바닥에 닿을 때까지 입력하지 않았을 경우, 단어의 y좌표 값을 더하여 아래로 떨어지도록 한다
            if w.pt.y() < self.rect.bottom():
                ...
                self.main.noTyping = True       #사용자가 주어진 단어 입력 실패를 나타내고 악어의 이동 여부를 나타내는 변수로서 True일 경우, 악어가 전진한다
