...
class CWidget(QWidget):
    ...
    def initUI(self):
        ...
        self.setGeometry(100, 100, 500, 500)    #.setGeometry=>윈도우의 시작 위치와 크기를 설정한다/윈도우는 (100, 100) 위치에서 시작하고 크기는 가로 500, 세로 500로 설정한다
    ...
    def paintEvent(self, e):
        ...
        qp.begin(self)     #QpaintDevice 여기서 self는 QWidget/ .begin()=>boolean형태를 리턴하며 성공적으로 그림그리기 장치를 불러왔는지 확인한다
    ...
    def toggleButton(self, state):
        ...
        if state:
            self.map.gameStart(self.lang.currentIndex(), self.level.currentIndex())  #.currentIndex())=>QComboBox의 현재 인덱스를 가져온다. 사용자가 선택한 언어와 난이도에 따라 self.map에 있는 gameStart함수를 실행한다
            self.btn.setText('게임종료')    # .setTExt(')btn의 텍스트를 설정한다
            self.lang.setEnabled(False)    # .setEnabled(boolean)=>QComboBox 무력화할지 결정한다. 메소드의 인자 값이 음수일 경우 QComboBox를 무력화한다
            ...
            self.lang.setEnabled(True)    # .setEnabled(boolean)=>QComboBox 무력화할지 결정한다. 메소드의 인자 값이 음수일 경우 QComboBox를 무력화한다
